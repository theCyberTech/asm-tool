...
class EmailEnumerator:
    ...
    def enumerate(self, domain: str) -> Dict:
        """
        Enumerate emails for a domain using multiple sources.
        ...
        if hunter_key:
            hunter_emails, hunter_meta = self._query_hunter(domain, hunter_key)
            if hunter_emails:
                all_emails.update(hunter_emails)
                results['by_source']['hunter.io'] = list(hunter_emails)
                sources_used.append('hunter.io')
                if hunter_meta.get('pattern'):
                    results['patterns'].append(hunter_meta['pattern'])

        # Phonebook.cz (free)
        phonebook_emails = self._query_phonebook(domain)
        if phonebook_emails:
            all_emails.update(phonebook_emails)
            results['by_source']['phonebook.cz'] = list(phonebook_emails)
            sources_used.append('phonebook.cz')

        # EmailRep.io (free, limited)
        # Skipping - requires email input, not domain search

        # Skymem (free)
        skymem_emails = self._query_skymem(domain)
        if skymem_emails:
            all_emails.update(skymem_emails)
            results['by_source']['skymem.info'] = list(skymem_emails)
            sources_used.append('skymem.info')

        # Google dorking simulation - search for emails in CT logs
        ct_emails = self._search_ct_logs(domain)
        if ct_emails:
            all_emails.update(ct_emails)
            results['by_source']['ct_logs'] = list(ct_emails)
            sources_used.append('ct_logs')
    ...
    def _query_phonebook(self, domain: str) -> Set[str]:
        ...
        try:
            url = f'https://phonebook.cz/api.php'
            data = {
                'input': domain,
                'type': 'email',
            }
    ...
    def verify_email(self, email: str) -> Dict:
        ...
        role_prefixes = [
            ...
            'help', 'billing', 'hr', 'jobs', 'careers',
            'press', 'media', 'marketing', 'legal', 'abuse',
            'security', 'privacy', 'compliance', 'feedback',
        ]
    ...
    def enumerate_multiple(self, domains: List[str], workers: int = 5) -> Dict:
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {
                executor.submit(self.enumerate, domain): domain
